<template>
  <div class="pdf-preview">
    <van-nav-bar :title="title" :left-text="$t('pdfPreview.back')" left-arrow @click-left="onClickLeft" />
    ...
    <div class="pdf-container" ref="pdfContainer">
      ...
      <div v-if="error" class="error-container">
        <van-empty image="error" :description="$t('pdfPreview.loadFailed')" />
      ...
      <div v-if="!loading && !error" class="pdf-content">
        <div class="pdf-controls">
          ...
          <van-button size="small" icon="arrow-up" @click="scrollToTop" v-if="totalPages > 1"
            :title="$t('pdfPreview.home')" />
          ...
          <van-button size="small" icon="down" @click="downloadPdf" type="primary" :title="$t('pdfPreview.download')" />
        </div>
  ...
  const fitToWidth = async () => {
    ...
    try {
      ...
      if (canvasContainer.value) {
        ...
        // 减去左右 padding (32px) + 预留垂直滚动条宽度 (17px)
        const scrollbarWidth = 17
        const availableWidth = containerWidth - 32 - scrollbarWidth
